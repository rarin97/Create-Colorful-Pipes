...
public class ColorfulDrainItemHandler implements IItemHandler {

	private ColorfulDrainBlockEntity blockEntity;
	private Direction side;

	public ColorfulDrainItemHandler(ColorfulDrainBlockEntity be, Direction side) {
		this.blockEntity = be;
		this.side = side;
	}
	...
	public ItemStack extractItem(int slot, int amount, boolean simulate) {
		...
		ItemStack extracted = heldStack.copy();
		extracted.setCount(Math.min(amount, heldStack.getCount()));

		if (!simulate) {
			heldStack.shrink(extracted.getCount());
			blockEntity.notifyUpdate();
		}
