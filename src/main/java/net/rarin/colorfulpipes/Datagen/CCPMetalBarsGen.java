...
public class CCPMetalBarsGen {
	...
	public static BlockEntry<IronBarsBlock> createBars(String name, boolean specialEdge, MapColor color) {
		return ColorfulPipes.registrate().block(name + "_bars", IronBarsBlock::new)
				.addLayer(() -> RenderType::cutoutMipped)
				.initialProperties(() -> Blocks.IRON_BARS)
				.properties(p -> p.sound(SoundType.COPPER).mapColor(color))
				.tag(AllTags.AllBlockTags.WRENCH_PICKUP.tag)
				.tag(AllTags.AllBlockTags.FAN_TRANSPARENT.tag)
				.tag(CCPTags.ColorfulBlockTags.COLORFUL_COPPER_BARS.tag)
				.transform(TagGen.pickaxeOnly())
				.blockstate(barsBlockState(name, specialEdge))
				.item()
				.tag(CCPTags.ColorfulItemTags.COLORFUL_COPPER_BARS.tag)
				.tag(CCPTags.ColorfulItemTags.COPPER_BARS.tag)
				.model((c, p) -> {
					ResourceLocation barsTexture = p.modLoc("block/bars/" + name + "_bars");
