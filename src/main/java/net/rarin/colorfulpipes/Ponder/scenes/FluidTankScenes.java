...
public class FluidTankScenes {
	...
	public static void storage(SceneBuilder builder, SceneBuildingUtil util) {
		...
		scene.world().modifyBlock(pumpPos, s -> s.setValue(ColorfulPumpBlock.FACING, Direction.SOUTH), false);
		scene.idle(10);
		...
		scene.world().modifyBlock(pumpPos, s -> s.setValue(ColorfulPumpBlock.FACING, Direction.NORTH), true);
		scene.world().propagatePipeChange(pumpPos);
		for (int i = 0; i < 4; i++) {
			scene.world().modifyBlockEntity(tankPos, FluidTankBlockEntity.class, be -> be.getTankInventory()
